    ...
    instance.AddTab("Dev", _ => { 
      if (ImGui.CollapsingHeader("Dev信息")) {
        if (ECHelper.ClientState.LocalPlayer is not null) {
          ...
          ImGui.Text($"Target DeathPrediction: {MobPullManager.GetTargetTTK()}");
      ...
      if (ImGui.CollapsingHeader("更新日志")) {
        ...
        ImGui.Text(
          ...
          + "2025/10/06: 尝试修复拉怪中红条溢出的bug。\n"
          + "2025/10/07: 修复偶尔附体会吞掉完人的bug，改进自动资源倾泻，重做UI标签栏\n"
          + "2025/10/22: 修复回退Hotkey，修改距离判断方式");
        ImGui.EndChild();
      }
    });
  }
